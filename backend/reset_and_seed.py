...
def seed_stress_test_data():
    """Seed 4 additional stress-test locations: cardiology, orthopedics, women's health, pediatrics."""
    from app.models import (User, Location, Appointment, BiomarkerReading, MedicalHistory)
    from sqlalchemy.exc import IntegrityError

    if Location.query.filter_by(name="Piedmont Heart Institute").first():
        print("[OK] Stress test locations already seeded, skipping.")
        return
    ...
    LOCATIONS_CONFIG = [
        {
            ...
            "staff": [
                ...
                ("James", "Holloway", "Dr."), ("Priya", "Nair", "RN"),
                ("Kevin", "Moss", "Dr."),
            ],
            "patients": [
        ...
        },
        {
            ...
            "staff": [
                ...
                ("Tanya", "Brooks", "CNM"),   ("Laura", "Simmons", "Dr."),
                ("Michelle", "Park", "Dr."),
            ],
            "patients": [
        ...
        },
        {
            ...
            "staff": [
                ...
                ("Brendan", "Walsh", "NP"),     ("Stephanie", "Morris", "Dr."),
                ("Justin", "Palmer", "Dr."),
            ],
            "patients": [
    ...
    for loc_cfg in LOCATIONS_CONFIG:
        ...
        all_user_emails.append(f"  Location Admin: {admin.email}")
        ...
        for staff_idx, (sfirst, slast, stitle) in enumerate(loc_cfg["staff"]):
            ...
            db.session.flush()
            all_user_emails.append(f"  Staff:          {staff_user.email}")

            for pat_idx, (pfirst, plast) in enumerate(loc_cfg["patients"][staff_idx]):
                pat_addr = f"{loc_cfg['addresses'][addr_idx]}, Charlotte, NC"
                addr_idx += 1
                ...
                db.session.add(patient)
                db.session.flush()
                all_user_emails.append(f"  Patient:        {patient.email}")

                # Medical history — 2 distinct conditions, rotated by position
                ...
                # 3 completed appointments, staggered 270 / 180 / 90 days ago
                ...
                for appt_num in range(3):
                    ...
                    # 3–5 biomarkers per appointment, values descend slightly over time
                    num_bm = 3 + (appt_num % 2)
                    for bm_key in bm_keys[:num_bm]:
                        ...
                        raw = lo + frac * (hi - lo)
                        # Round nicely based on expected precision
    ...
    try:
        db.session.commit()
        print(f"[OK] Stress test data seeded: 4 locations, 20 staff, 100 patients, 300 completed appointments.")
        print(f"     All passwords: 'password'")
        print(f"     Email format:  firstname.lastname@locationdomain.com")
        print(f"     Domains: piedmontheart | southparkorthopedic | carolinawomenshealth | uptownpediatrics")
    except IntegrityError as e:
        db.session.rollback()
        print(f"[WARN] Stress test seed failed: {e}")
